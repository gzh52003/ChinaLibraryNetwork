<template>
  <div>
    <div style="margin-top: 15px;">
      <el-input placeholder="请输入内容" v-model="inputValue" class="input-with-select" style="width:30%">
        <el-select v-model="selectType" slot="prepend" placeholder="请选择" @change="getuserlistBytype">
...
async function getUserInfoList($request,params){
  ...
  console.log(data);
  let res = {}
  res.data = data.data.data.map(item=>{item.gender= item.gender==='m'?"男":"女";item.role= item.role==='admin'?"管理员":"普通用户";return item});
...
export default {
  ...
  methods: {
    ...
    async multiDelete(){
      ...
      const res = await this.$request.delete("/userinfo",{params:{idarr:params}});
